...
class AddStockDialog(QtWidgets.QDialog):
    ...
    def accept(self):
        ...
        if not ticker:
            self.ui.labelError.setText(self.get_red_text('Error: Invalid Ticker'))
            self.ui.labelError.show()
            return
        for ii in range(len(self.stocks)):
            if self.stocks[ii].ticker == ticker:
                if self.stocks[ii].group == group:     
                    self.ui.labelError.setText(self.get_red_text('Error: Stock already in {0}'.format(group)))
                    self.ui.labelError.show()
                    return
                else:
